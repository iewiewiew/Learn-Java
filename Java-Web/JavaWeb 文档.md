...
2. [spring-boot-demo](https://gitee.com/roncoocom/spring-boot-demo)
...
### 1.1、配置数据库
1、jdbc配置
jdbc配置：src/resources/application.properties
```
...
```
2、sql语句
sql语句路径：src/resources/dbname.sql
执行dbname.sql进行创库、建表、导数据

### 1.2、启动JavaWeb
1、启动JavaWeb
- 方式一：双击bin/start.bat（Windows） / sh bin/start.sh（Mac）
- 方式二：进入Java-Web的pom.xml所在目录，执行：`mvn clean` `mvn package` `java -jar ./target/Java-Web-1.0-SNAPSHOT.jar`
- 方式三：在IDEA双击Maven的clean、install，运行com/wmh/JavaWebApplication.java
- 方式四：k8s
```
...
```
2、验证：在浏览器访问 http://127.0.0.1:9999/demo/list 或者 http://127.0.0.1:9999/swagger-ui.html#/
3、端口号被占用解决方式：打开cmd执行：netstat -ano | findstr 9999，找到9999的pid，taskkill /f /pid 55852， 或者打开任务管理器-详细信息，根据pid杀掉进程

### 1.3、接口地址
1. 丝袜哥地址：http://127.0.0.1:9999/swagger-ui.html#/
2. demo列表地址：http://127.0.0.1:9999/demo/list
3. 根据id查找demo：http://127.0.0.1:9999/demo/list/1
4. 新增demo地址：http://127.0.0.1:9999/demo/add
```
{
    "id":"1",
...
5. 更新demo地址：http://127.0.0.1:9999/demo/update
```
{
    "id":"1",
    "username": "usernameup",
...
```
6. 删除demo地址：http://127.0.0.1:9999/demo/delete?id=1

7. 上传文件地址：http://127.0.0.1:9999/uploadpage

8. dubbo接口地址：http://127.0.0.1:9999/sayHello

9. redis接口地址：http://127.0.0.1:9999/redis

10. 客户端发送内容：http://127.0.0.1:63342/Learn-Java/Java-Web/com/wmh/view/socket.html

11. 服务端发送内容：http://127.0.0.1:9999/wbtest/sendMsg/test_websocket

### 1.4、接口文档
Spring Boot 框架集成Knife4j：https://doc.xiaominfo.com/docs/action/springboot
接口文档：http://127.0.0.1:9999/doc.html#/home
踩坑1：问题：SpringBoot使用knife4j无法引入@EnableSwagger2WebMvc，解决：
把原先的swagger2注释掉，点击Maven的刷新按钮并重新clean install。  
踩坑2：问题：启动报错，解决：升级SpringBoot版本到2.3.5.RELEASE。
...
### 2.2、参数校验
参数绑定：在Controller使用方法参数接收值，就是把web端的值给接收到Controller中处理，这个过程就叫做参数绑定。
在controller层做参数校验
...
### 2.3、其它
1、控制台打印sql  
    在application.yml添加logging配置, com.wmh.dao是dao层包路径
```
logging:
  level:
    com.wmh.dao: debug
...
验证：http://127.0.0.1:8001/hello?name=demo
